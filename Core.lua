        ...
        searcher:SetResultsUpdatedCallback(function()
            ...
            for _, entryID in ipairs(entries or {}) do
                ...
                if owned and type(info.recordID) == "number" and type(info.name) == "string" and info.name ~= "" then
                    ownedEntries = ownedEntries + 1
                    if #items < 1024 then items[#items + 1] = {
                        decorID = info.recordID, itemID = info.itemID, name = info.name,
                        icon = info.iconTexture, storedCount = info.totalNumStored,
                        placedCount = info.totalNumPlaced, redeemableCount = info.remainingRedeemable,
                        destroyableCount = info.destroyableInstanceCount, uniqueTrophy = info.isUniqueTrophy,
                        allowedIndoors = info.isAllowedIndoors, allowedOutdoors = info.isAllowedOutdoors,
                    } end
                end
            end
            ...
            ns.decorInventory = {
                available = true, capturedAt = time(), items = items,
